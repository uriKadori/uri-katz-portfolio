...
document.querySelector('#projects').innerHTML = projects.map((p,i) => `<article class="project"><div class="project-picture"><img src="assets/${p.image}" alt="${p.title} gameplay" loading="lazy"><span class="project-number">${String(i+2).padStart(2,'0')}</span></div><div class="project-meta"><h3>${p.title}</h3><span class="platform">${p.platform}</span></div><p>${p.description}</p><div class="project-links">${p.links.map(([label,url])=>`<a href="${url}" target="_blank" rel="noopener noreferrer">${label} ↗</a>`).join('')}</div></article>`).join('');
